        ...
        int flags)
{
    ...
    if(codec_data->codec_context->codec_type == AVMEDIA_TYPE_VIDEO){
        AVRational video_time = mp4_ctx->streams[stream_index]->time_base;  // 1/15360
        AVRational time_base = codec_data->codec_context->time_base;  // 1/30
        uint64_t duration = pts - (uint64_t)(last_pts  * 1000 * 1000000 / time_base.den);

        //time_base为1/30, 每帧数据应该占33毫秒，实际上每次采集的数据长度在一帧到两帧之间，采用浮点数记录递增后的pts，写入时再转为整数。
        video_f_pts += (double)duration / (1000 / time_base.den * 1000000);
        video_pts = (int64_t)(video_f_pts);
        //printf("video_f_pts = %lf, video_pts = %ld.\n", video_f_pts, video_pts);

        if (video_pts == last_pts)  //pts musit be incremented, but not necessarily continuous.
            video_pts++;
